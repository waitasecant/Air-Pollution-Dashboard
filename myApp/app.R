...
paramtab= c("PM2.5", "PM10", "NH3", "SO2")
cutofftab = c(60,100,400,80)
naaqstab = data.frame(list("Pollutant"=paramtab, "NAAQS Standard (24hr/8hr/1hr)"=as.integer(cutofftab)), check.names = FALSE)


aqitab = data.frame(list("AQI"="Range", "Good"="0-50", "Satisfactory"="51-100", "Moderate"="101-200",
                    "Poor"="201-300", "Very Poor"="301-400", "Severe"=">401"), check.names = FALSE)
...
getRangeDataTS = function(df, par, start, end, site){
  if (par %in% c("AQI")){
    ...
    return(getPartitionsAQI(df, par))
  }
...
getPartitionsAQI = function(df, par){

  y0 = df[,par]
  y0 = y0[1:(length(y0)-1)]

  colourCodeAQI = ifelse(y0>401, "darkred", ifelse(301<y0, "red", ifelse(201<y0, "orange",
  ...
  for (i in 1:length(colourCodeAQI)){
    if (colourCodeAQI[i]=="darkred"){
    ...
    else if (colourCodeAQI[i]=="yellow2"){
    ...
    else if (colourCodeAQI[i]=="green3"){
    ...
    else if (colourCodeAQI[i]=="darkgreen"){
...
getPartitions = function(df, par){
  ...
  colourCode = ifelse(y0>getCutoff(par), "red", "green")
  redx0 = c()
  redx1 = c()
  redy0 = c()
  redy1 = c()
  ...
  redPartitions = list("x" = redx0, "xend"= redx1, "y"= redy0, "yend"= redy1)
  greenPartitions = list("x" = greenx0, "xend"= greenx1, "y"= greeny0, "yend"= greeny1)
  partitions = list("redp" = redPartitions, "greenp" = greenPartitions)
  return(partitions)
...
hoverName = function(par, col){
  if (par!="AQI"){
    if (col == "redp"){
      return(paste(isolate(par),">", getCutoff(isolate(par)), sep=""))
    }
    else{
      return(paste(isolate(par),"<", getCutoff(isolate(par)), sep=""))
    }
...
ui = fluidPage(
  ...
  # -------------Dashboard Page ------------------------------------------------
    navbarMenu("Dashboard",
      tabPanel("Choropleth Maps",
        fluidRow(column(3,offset=0.5),strong(p("Choropleth map of monthly average of pollutant parameter",
                                          align="center",style = "font-size:18pt"))),

        fluidRow(column(3, offset= 0.5,
                        selectInput("yearMap", "Year", c(2023, 2022), width="250pt"),
                        selectInput("monthMap", "Month", choices = NULL, width="250pt"),
                        selectInput("paramMap", "Pollutant", c("AQI","PM2.5", "PM10", "NH3", "SO2"), width="250pt"),
                        actionButton("plotbuttonCM", "PLOT",class = "btn-sm btn-primary", width="250pt")
                        ),
                 column(4,offset=0.5,imageOutput("monthlyAvgGlobal")|>withSpinner(color="black"),br(),br(),hr(),
                        p("Monthly average on fixed scale of values observed.",align="center",style = "font-size:12pt")),
                 column(4,offset=0.5,imageOutput("monthlyAvgLocal")|>withSpinner(color="black"),br(),br(),hr(),
                        p("Monthly average on scale for the selected month.",align="center",style = "font-size:12pt"))
                )
       ),
      tabPanel("Time-Series Plots",
               fluidRow(column(3,offset=0.5),strong(p("Time-series plot of daily average of pollutant parameter",
                                               align="center",style = "font-size:18pt"))),
               fluidRow(column(3, offset= 0.5,
                               selectInput("siteTS", "Station", getDelhiSites(df), width="250pt"),
                               selectInput("paramTS", "Pollutant", c("AQI","PM2.5", "PM10", "NH3", "SO2"), width="250pt"),
                               dateRangeInput("daterangeTS", "Date Range", start = max(df$Date)-30, end = max(df$Date),
                                              min = "2022-01-01", max = "2023-10-31", format="dd-mm-yyyy", width="250pt"),
                               checkboxInput("checkTS", "Fill the area under the curve", value = FALSE),
                               actionButton("plotbuttonTS", "PLOT",class = "btn-sm btn-primary", width="250pt"),
                               br(),br(),br(),
               ...
               column(9,offset=0.5, plotlyOutput("TSPlot")|>withSpinner(color="black"),
                      br(),br(),br(),br(),tableOutput("aqitable"))
               )
       ),
      tabPanel("Last 24-hr Plots",
               fluidRow(column(3,offset=0.5),strong(p("Time-series plot of last 24-hr of pollutant parameter",
                                                      align="center",style = "font-size:18pt"))),
               fluidRow(column(3, offset= 0.5,
                               selectInput("site24", "Station", getDelhiSites(df24), width="250pt"),
                               selectInput("param24", "Pollutant", c("PM2.5", "PM10", "NH3", "SO2"), width="250pt"),
                               checkboxInput("check24", "Fill the area under the curve", value = FALSE),
                               actionButton("plotbutton24", "PLOT",class = "btn-sm btn-primary", width="250pt"),
                               br(),br(),br(),br(),
                               div(style = "margin: auto; font-size:12pt; text-align:left",
                                   strong(p("National Ambient Air Quality Standard (\\(\\mu g/m^{3}\\))"))),
                               tableOutput("naaqstable24")
                               ),
               column(9,offset=0.5, plotlyOutput("TSPlot24")|>withSpinner(color="black"))
               )
      )
     ),
  # -----------Temp-Home Page----------------------------------------------------
  ...
  #          titlePanel(strong(p("Welcome to the Delhi Air Pollution Dashboard", align="center", style = "font-size:28pt"))),
  #          fluidRow(br()),
  #          fluidRow(column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h")),
  #          fluidRow(column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h")),
  #          fluidRow(column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h")),
  #          fluidRow(column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h"),column(2, "h"))
  #  ),
...
server = function(input, output){
  ...
  output$naaqstable <- renderTable({
    naaqstab
  },bordered = TRUE, striped=TRUE, width="250pt")

  output$aqitable <- renderTable({
    aqitab
  },bordered = TRUE, striped=TRUE, width="800pt")
  ...
  output$TSPlot = renderPlotly({
    fig = plot_ly(height= 460, type="scatter", mode="markers")
  ...
  observeEvent(input$plotbutton24,{
    ...
    output$TSPlot24 = renderPlotly({
      # -------------TSPlot24---------------------------------------------------
      if (input$check24==T){
        fig = plot_ly(height= 560, type="scatter", mode = "markers", x = rangeDataTS24Pt()$Date, y = rangeDataTS24Pt()$par)
        for (j in emptyCol){
